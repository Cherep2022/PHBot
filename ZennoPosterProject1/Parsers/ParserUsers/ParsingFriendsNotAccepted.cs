...
namespace ZennoPosterProject1.Parsers.ParserUsers
{
    internal class ParsingFriendsNotAccepted
    {
        ...
        public void StartFriendsNoAcceptedParsing()
        {
            ...
            url = "https://rt.pornhub.com/user/friend_requests_ajax?sent=1&token=MTY3NDA2MzkzMJ0xnDxQCpN6sPF-9N_QqF0JLEQT3unC9T9ADAWHlIuhDm_bs_O7JgmcqWy9srOMNYc1maH6LSCvvSb8lnu6n0o";
            resultHttpGet = ZennoPoster.HttpGet(url, "", "UTF-8", ZennoLab.InterfacesLibrary.Enums.Http.ResponceType.BodyOnly, 5000, "", project.Profile.UserAgent, true, 5, nums, "", true);
            ...
            {
                dataBaseModelContainer.Database.ExecuteSqlCommand("TRUNCATE TABLE [FriendNotAccepted]");

                foreach (var htmlNode in htmlNodeCollection)
                {
                    FriendNotAccepted friendNotAccepted = new FriendNotAccepted();
                    friendNotAccepted.UserName = htmlNode.InnerText.Split('\\')[0].ToLower().Replace(" ", "-");
                    dataBaseModelContainer.FriendNotAccepteds.Add(friendNotAccepted);
                }
